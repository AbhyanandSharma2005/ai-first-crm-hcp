from datetime import datetime, timedelta
import re

from database import SessionLocal
from models import Interaction

from services.groq_service import groq_service
from services.json_parser import parse_llm_json
from services.session_memory import session_memory

from prompts.interaction_prompt import (
    SYSTEM_PROMPT,
    FOLLOW_UP_PROMPT,
)


# ============================================================
# Helper Function
# ============================================================

def parse_relative_date(text: str):
    """
    Converts relative follow-up phrases into a date.

    Supported examples:

    after two weeks
    after 2 weeks
    after 15 days
    after one month
    tomorrow
    next week

    Returns:
        datetime.date | None
    """

    if not text:
        return None

    text = text.lower().strip()

    today = datetime.today().date()

    # ----------------------------
    # Tomorrow
    # ----------------------------

    if "tomorrow" in text:
        return today + timedelta(days=1)

    # ----------------------------
    # Next week
    # ----------------------------

    if "next week" in text:
        return today + timedelta(days=7)

    # ----------------------------
    # After X days
    # ----------------------------

    match = re.search(
        r"after\s+(\d+)\s+day",
        text
    )

    if match:

        days = int(match.group(1))

        return today + timedelta(days=days)

    # ----------------------------
    # After X weeks
    # ----------------------------

    match = re.search(
        r"after\s+(\d+)\s+week",
        text
    )

    if match:

        weeks = int(match.group(1))

        return today + timedelta(weeks=weeks)

    # ----------------------------
    # After two weeks
    # ----------------------------

    if "after two weeks" in text:

        return today + timedelta(weeks=2)

    if "after one week" in text:

        return today + timedelta(weeks=1)

    # ----------------------------
    # After one month
    # ----------------------------

    if "after one month" in text:

        return today + timedelta(days=30)

    return None


# ============================================================
# Follow-up Scheduler Tool
# ============================================================

def follow_up_scheduler_tool(state: dict) -> dict:
    """
    Schedule or update a follow-up.

    If the HCP is omitted,
    Session Memory is used.

    Supports:

    • YYYY-MM-DD
    • after two weeks
    • after 15 days
    • tomorrow
    • next week
    """

    message = state["user_message"]

    prompt = FOLLOW_UP_PROMPT.format(
        message=message
    )

    db = SessionLocal()

    try:

        response = groq_service.chat(
            system_prompt=SYSTEM_PROMPT,
            user_prompt=prompt
        )

        print("\n========== GROQ RESPONSE ==========")
        print(response)
        print("===================================\n")

        data = parse_llm_json(response)

        session_id = state.get("session_id")

        # =====================================================
        # HCP NAME
        # =====================================================

        hcp_name = data.get(
            "hcp_name",
            ""
        ).strip()

        print("LLM extracted HCP:", hcp_name)

        if not hcp_name and session_id:

            print("Using Session Memory...")

            hcp_name = session_memory.get_value(
                session_id,
                "last_hcp"
            )

            if hcp_name:

                hcp_name = hcp_name.strip()

        print("Final HCP:", hcp_name)

        if not hcp_name:

            return {
                "tool_result": {
                    "status": "error",
                    "message": "No HCP name found."
                }
            }

        # =====================================================
        # FOLLOW-UP DATE
        # =====================================================

        follow_up_str = data.get(
            "follow_up",
            ""
        ).strip()

        # -----------------------------------------------------
        # First try LLM date
        # -----------------------------------------------------

        follow_up_date = None

        if follow_up_str:

            try:

                follow_up_date = datetime.strptime(
                    follow_up_str,
                    "%Y-%m-%d"
                ).date()

            except ValueError:

                follow_up_date = parse_relative_date(
                    follow_up_str
                )

        # -----------------------------------------------------
        # If LLM returned nothing,
        # parse the original user message
        # -----------------------------------------------------

        if follow_up_date is None:

            follow_up_date = parse_relative_date(
                message
            )

        if follow_up_date is None:

            return {

                "tool_result": {

                    "status": "error",

                    "message": (
                        "Unable to determine the follow-up date."
                    )

                }

            }

        print("Resolved Follow-up Date:", follow_up_date)

        # =====================================================
        # Find Latest Interaction
        # =====================================================

        interaction = (
            db.query(Interaction)
            .filter(
                Interaction.hcp_name.ilike(
                    f"%{hcp_name}%"
                )
            )
            .order_by(
                Interaction.id.desc()
            )
            .first()
        )

        if interaction is None:

            return {

                "tool_result": {

                    "status": "error",

                    "message": (
                        f"No interaction found for {hcp_name}."
                    )

                }

            }

                    # =====================================================
        # Update Database
        # =====================================================

        interaction.follow_up = follow_up_date

        db.commit()

        db.refresh(interaction)

        print("\n========== DATABASE UPDATED ==========")
        print("Interaction ID :", interaction.id)
        print("HCP            :", interaction.hcp_name)
        print("Follow-up      :", interaction.follow_up)
        print("======================================\n")

        # =====================================================
        # Update Session Memory
        # =====================================================

        if session_id:

            session_memory.set_value(
                session_id,
                "last_hcp",
                interaction.hcp_name
            )

            session_memory.set_value(
                session_id,
                "last_follow_up",
                str(interaction.follow_up)
            )

        # =====================================================
        # Return Success
        # =====================================================

        return {

            "tool_result": {

                "status": "success",

                "interaction_id": interaction.id,

                "hcp_name": interaction.hcp_name,

                "follow_up": str(
                    interaction.follow_up
                ),

                "message": (
                    f"Follow-up scheduled for "
                    f"{interaction.hcp_name} on "
                    f"{interaction.follow_up}."
                )

            }

        }

    except Exception as e:

        db.rollback()

        print("\n========== FOLLOW-UP ERROR ==========")
        print(str(e))
        print("=====================================\n")

        return {

            "tool_result": {

                "status": "error",

                "message": str(e)

            }

        }

    finally:

        db.close()